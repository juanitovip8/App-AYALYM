  ...
  const svcsHtml=svcs.length
    ?svcs.map(ps=>{
      ...
      return`<div class="pi-svc-row" style="flex-direction:column;align-items:stretch;gap:0;padding:14px 16px;">
        ...
        ${(()=>{const hasPlan=ps.planTrabajo&&(ps.planTrabajo.filas||[]).length>0;if(!hasPlan)return'';return`<div style="margin-top:8px;padding:8px 12px;border-radius:8px;background:${_dark?'rgba(24,95,165,.10)':'#F0F6FF'};border:.5px solid ${_dark?'rgba(24,95,165,.3)':'#C5D8EC'};display:flex;align-items:center;justify-content:space-between;gap:8px;">
          <span style="font-size:11px;font-weight:700;color:${_dark?'#6BAAFF':'#185FA5'};">📋 Plan de Trabajo · ${ps.planTrabajo.filas.length} actividades</span>
          <button class="btn-sm" style="font-size:11px;padding:3px 10px;" onclick="abrirPlanTrabajo(${ps.id},false)">👁 Ver</button>
        </div>`;})()}
      </div>`;}).join('')
...
/* ══════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════ */
const _DIAS_PLAN=[
  {key:'L',  short:'L',  full:'Lunes'},
  {key:'Ma', short:'M',  full:'Martes'},
  {key:'Mi', short:'M',  full:'Miércoles'},
  {key:'J',  short:'J',  full:'Jueves'},
  {key:'V',  short:'V',  full:'Viernes'},
  {key:'S',  short:'S',  full:'Sábado'},
    ...
    <div style="overflow-x:auto;border:.5px solid rgba(255,255,255,.12);border-radius:10px;overflow:hidden;">
      <table style="width:100%;border-collapse:collapse;">
        <thead><tr style="background:#042C53;">
          ...
          <th style="padding:9px 12px;font-size:9.5px;font-weight:700;color:#fff;text-align:left;white-space:nowrap;">HORA FIN</th>
          <th style="padding:9px 12px;font-size:9.5px;font-weight:700;color:#fff;text-align:left;">ÁREA</th>
          <th style="padding:9px 12px;font-size:9.5px;font-weight:700;color:#fff;text-align:left;">ACTIVIDADES A REALIZAR</th>
          <th style="padding:9px 12px;font-size:9.5px;font-weight:700;color:#fff;text-align:left;">DÍAS</th>
        </tr></thead>
...
function guardarPlanTrabajo(psId,personalId){
  ...
  document.querySelectorAll('.pt-fila').forEach(el=>{
    const m=el.id.match(/pt-fila-(\d+)/);if(!m)return;
    const i=m[1];
    const horaInicio=(document.getElementById('pt-hi-'+i)||{}).value||'';
    const horaFin=(document.getElementById('pt-hf-'+i)||{}).value||'';
    const area=((document.getElementById('pt-area-'+i)||{}).value||'').trim();
    const actividades=((document.getElementById('pt-act-'+i)||{}).value||'').trim();
    const dias=_DIAS_PLAN.filter(d=>(document.getElementById('pt-d-'+i+'-'+d.key)||{}).checked).map(d=>d.key);
...
function _abrirPlanPDF(ps,plan,pi,logoB64){
  ...
  const rows=filas.map((f,i)=>`<tr style="background:${i%2===0?'#fff':'#F7FAFF'};">
    ...
    <td style="padding:8px 11px;font-size:11px;border-bottom:.5px solid #EBF1FA;font-weight:600;color:#042C53;">${f.area||'—'}</td>
    <td style="padding:8px 11px;font-size:11px;border-bottom:.5px solid #EBF1FA;line-height:1.55;color:#1C2B3A;">${f.actividades||'—'}</td>
    <td style="padding:8px 11px;font-size:11px;border-bottom:.5px solid #EBF1FA;color:#185FA5;font-weight:600;">${(f.dias||[]).map(d=>_DIAS_PLAN.find(x=>x.key===d)?.full||d).join(', ')}</td>
  </tr>`).join('');
...
.info{background:#F0F6FF;border-bottom:1.5px solid #D0E3F7;padding:9px 28px;display:flex;gap:22px;flex-wrap:wrap;}
.ib{display:flex;gap:5px;align-items:baseline;}.ibl{font-size:9px;color:#5C7A9A;font-weight:700;text-transform:uppercase;letter-spacing:.3px;}.ibv{font-size:11.5px;font-weight:700;color:#042C53;}
.body{padding:18px 28px 22px;}
table{width:100%;border-collapse:collapse;border:1px solid #D0E3F7;border-radius:8px;overflow:hidden;}
thead tr{background:#042C53;}th{padding:9px 11px;font-size:9.5px;font-weight:700;color:#fff;text-align:left;letter-spacing:.3px;}
.ftr{background:#F0F4FA;border-top:1px solid #D0E3F7;padding:7px 28px;display:flex;justify-content:space-between;align-items:center;margin-top:20px;}
.ftr span{font-size:9px;color:#8A9BB0;}.ftr strong{color:#1A56DB;}
...
function _cinmData(){
  ...
  const seen=new Set();
  const list=[];
  function _add(p){if(p&&!seen.has(p.id)){seen.add(p.id);list.push(p);}}
  /* 1. Por clienteInmId (campo directo en el PS) */
  PROPERTY_SERVICES.filter(p=>p.clienteInmId===ci.id).forEach(_add);
  /* 2. Por contratoId legacy */
  if(ci.contratoId!=null) _add(PROPERTY_SERVICES.find(p=>p.id===ci.contratoId));
  /* 3. Por email del cliente en ps.cliente.email (contratos históricos) */
  if(ci.email) PROPERTY_SERVICES.filter(p=>p.cliente&&p.cliente.email===ci.email).forEach(_add);
  /* Ordenar por folio */
  const psList=list.sort((a,b)=>(a.folio||'').localeCompare(b.folio||''));
  /* Auto-arreglar clienteInmId faltante (diferido para no bloquear render) */
  setTimeout(()=>{let fixed=false;psList.forEach(p=>{if(!p.clienteInmId){p.clienteInmId=ci.id;fixed=true;}});if(fixed)fbSavePropertyServices();},500);
...
function renderClienteInmInicio(){
  ...
  <div class="card">
    ...
    ${personal.length?personal.map(p=>`<div class="cinm-person-card">
      ...
      <div class="cinm-person-info"><p>${p.nombre}</p><span>Personal de inmuebles${p.tel?' \xB7 '+p.tel:''}</span><br><span style="font-size:11px;color:#5C7A9A;">${ps.folio}${ps.inmueble&&ps.inmueble.direccion?' — '+ps.inmueble.direccion.split(',')[0]:''}${ci.nombre?' — '+ci.nombre:''}</span></div>
...
function renderClienteInmContrato(){
  ...
  el.innerHTML=selector+`<div class="card">
    ...
    ${(ps.planTrabajo&&(ps.planTrabajo.filas||[]).length)?`<div class="div" style="margin:12px 0;"></div>
    <p class="cinm-section-title">📋 Plan de Trabajo</p>
    <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;padding:8px 12px;background:#EEF5FF;border-radius:8px;">
      <span style="font-size:12px;font-weight:600;color:#042C53;">${ps.planTrabajo.filas.length} actividades registradas</span>
      <button class="btn-sm" onclick="abrirPlanTrabajo(${ps.id},false)">👁 Ver plan</button>
    </div>`:''}
  </div>`;
}
...
function _renderAgCalendar(){
  ...
  const dateColors={};
  AGENDA_ITEMS.filter(a=>_agFilter==='all'?true:a.asignadoA?.rol===_agFilter).forEach(a=>{
    const aIni=a.fechaInicio||a.fecha||'';const aFin=a.fechaFin||aIni;
    if(!aIni)return;
    let d=new Date(aIni+'T12:00:00');const dEnd=new Date(aFin+'T12:00:00');
    while(d<=dEnd){
      const ds=d.toISOString().split('T')[0];
      if(!dateColors[ds])dateColors[ds]=new Set();
      dateColors[ds].add(AGENDA_ROL_COLORS[a.asignadoA?.rol]||'#6B7280');
...
function _renderAgDayEvents(ctx){
  ...
  const items=AGENDA_ITEMS
    .filter(a=>{
      if(ctx==='admin')return _agFilter==='all'?true:a.asignadoA?.rol===_agFilter;
      if(ctx==='supervisor'){if(!currentSupervisorRef)return false;const myIds=currentSupervisorRef.assignedWorkers||[];return(a.asignadoA?.rol==='trabajador'&&myIds.includes(a.asignadoA.id))||(a.asignadoA?.rol==='supervisor'&&a.asignadoA.id===currentSupervisorRef.id)||(a.asignadoA?.rol==='personal_inm');}
      return false;
    })
    .filter(a=>{const aIni=a.fechaInicio||a.fecha||'';const aFin=a.fechaFin||aIni;return ds>=aIni&&ds<=aFin;})
...
function openAgendaModal(id){
  ...
  modal.innerHTML=`<div class="confirm-card" style="max-width:500px;max-height:90vh;overflow-y:auto;">
    ...
    <div style="display:flex;flex-direction:column;gap:10px;">
      ...
      <div class="fld"><label>Tipo de servicio</label><select id="ag-tipo">
        ${svcTypes.map(s=>`<option value="${s.id}"${(item?.tipo===s.id)?' selected':''}>${_esc(s.nombre)}</option>`).join('')}
        <option value="otro"${(item?.tipo==='otro'||!item)?' selected':''}>Otro / Especial</option>
      </select></div>
      ...
      <div class="frow">
        <div class="fld"><label>Fecha inicio *</label><input type="date" id="ag-fecha-ini" value="${item?.fechaInicio||item?.fecha||todayISO}" onchange="_checkAgDisp()"></div>
        <div class="fld"><label>Fecha fin</label><input type="date" id="ag-fecha-fin" value="${item?.fechaFin||item?.fechaInicio||item?.fecha||todayISO}" onchange="_checkAgDisp()"></div>
      </div>
      <div style="font-size:11px;color:#5C7A9A;margin-top:-6px;">Las horas aplican a cada día del rango seleccionado.</div>
      <div class="frow">
...
function _checkAgDisp(){
  ...
  if(sinAsignarCb&&sinAsignarCb.checked){return;}
  const fechaIni=(document.getElementById('ag-fecha-ini')||{}).value||'';
  const fechaFin=(document.getElementById('ag-fecha-fin')||{}).value||fechaIni;
  ...
  if(!aVal||!fechaIni){div.style.display='none';return;}
  const[rol,idStr]=aVal.split(':');const id=parseInt(idStr);
  /* ── Conflictos en AGENDA_ITEMS ── */
  const conflicts=AGENDA_ITEMS.filter(a=>{
    ...
    const aIni=a.fechaInicio||a.fecha||'',aFin=a.fechaFin||aIni;
    if(aFin<fechaIni||aIni>fechaFin)return false; /* rangos de fecha no solapan */
    if(!a.horaInicio)return true; /* sin hora → consideramos conflicto de día */
    const aH=a.horaInicio,bH=a.horaFin||'23:59';
    const cI=horaIni||'00:00',cF=horaFin;
    return!(cF<=aH||cI>=bH);
  });
  /* ── Conflicto de horario de personal_inm con sus servicios asignados ── */
  let piScheduleWarn=false;
  if(rol==='personal_inm'&&horaIni){
    const pi=PERSONAL_INM.find(x=>x.id===id);
    if(pi){
      const piSvcs=PROPERTY_SERVICES.filter(ps=>(pi.serviciosAsignados||[]).includes(ps.id));
      const DAY_MAP={0:'dom',1:'lun',2:'mar',3:'mie',4:'jue',5:'vie',6:'sab'};
      let cur=new Date(fechaIni+'T12:00:00');const end=new Date(fechaFin+'T12:00:00');
      while(cur<=end&&!piScheduleWarn){
        const dk=DAY_MAP[cur.getDay()];
        for(const ps of piSvcs){
          if((ps.diasServicio||[]).includes(dk)){
            const psI=ps.hora||'00:00',psF=ps.horaSalida||'23:59';
            const cI=horaIni||'00:00',cF=horaFin;
            if(!(cF<=psI||cI>=psF)){piScheduleWarn=true;break;}
          }
        }
  ...
  /* ── Trabajador con servicios de reserva en rango ── */
  let wConf=false;
  if(rol==='trabajador'){
    const todayISO=new Date().toISOString().split('T')[0];
    const w=WORKERS.find(x=>x.id===id);
    if(w&&w.todayJobs?.length&&fechaIni<=todayISO&&fechaFin>=todayISO)wConf=true;
  }
  div.style.display='block';
  const warns=[];
  if(conflicts.length)warns.push(`⚠️ Tiene ${conflicts.length} servicio(s) en agenda con conflicto de horario.`);
  if(piScheduleWarn)warns.push('⚠️ Coincide con su horario de trabajo asignado en inmuebles.');
  if(wConf)warns.push('⚠️ Tiene servicios de reserva activos en algún día del rango.');
  if(warns.length){
    div.style.cssText='display:block;padding:8px 10px;border-radius:8px;font-size:12px;background:#FEF3C7;color:#92400E;';
    div.innerHTML=warns.join('<br>');
  } else {
    div.style.cssText='display:block;padding:8px 10px;border-radius:8px;font-size:12px;background:#D1FAE5;color:#065F46;';
    div.innerHTML='✅ Disponible en el rango de fechas/horario seleccionado';
  }
}
...
function saveAgendaItem(){
  ...
  const aVal=(document.getElementById('ag-asignado')||{}).value||'';
  const fechaIni=(document.getElementById('ag-fecha-ini')||{}).value||new Date().toISOString().split('T')[0];
  const fechaFinV=(document.getElementById('ag-fecha-fin')||{}).value||fechaIni;
  const horaIni=(document.getElementById('ag-hora-ini')||{}).value||'';
  ...
  if(!fechaIni){showToast('amber','⚠️','Selecciona una fecha de inicio');return;}
  ...
  if(_agEditId!=null){
    ...
    if(idx>=0){Object.assign(AGENDA_ITEMS[idx],{titulo,tipo,fecha:fechaIni,fechaInicio:fechaIni,fechaFin:fechaFinV,horaInicio:horaIni,horaFin,notas,estado,asignadoA:{id:assigneeId,nombre,rol}});fbSaveAgendaItem(AGENDA_ITEMS[idx]);showToast('green','✅','Servicio actualizado');}
  } else {
    const newId=_agNextId();
    const it={id:newId,titulo,tipo,descripcion:'',fecha:fechaIni,fechaInicio:fechaIni,fechaFin:fechaFinV,horaInicio:horaIni,horaFin,notas,estado:'pendiente',asignadoA:{id:assigneeId,nombre,rol},asignadoPor:{id:porId,nombre:porNombre,rol:porRol},redirectedBy:null,createdAt:Date.now()};
    ...
    pushNotif(notifRol,'📅','blue',`Nuevo servicio asignado: ${titulo}`,`Fecha: ${fechaIni}${horaIni?' a las '+horaIni:''}`);
